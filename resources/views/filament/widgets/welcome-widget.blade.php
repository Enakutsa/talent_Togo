    ...
    <div style="position:relative;display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:16px;padding:24px;">
        ...
        <div style="display:flex;gap:8px;flex-shrink:0;">
            <a
                href="{{ url('/') }}"
                target="_blank"
                style="display:inline-flex;align-items:center;gap:6px;border-radius:8px;border:1px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.05);padding:8px 16px;font-size:13px;font-weight:600;color:#e5e7eb;text-decoration:none;"
            >
                Voir le site
            </a>
            ...
            <button
                ...
                style="display:inline-flex;align-items:center;gap:6px;border-radius:8px;border:none;background:#fff;padding:8px 16px;font-size:13px;font-weight:600;color:#18181b;cursor:pointer;"
            >
                Déconnexion
            </button>
        </div>
...
<form id="admin-logout-form" method="POST" action="{{ route('filament.admin.auth.logout') }}" style="display:none;">
    @csrf
</form>